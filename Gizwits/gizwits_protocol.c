...
uint32_t ICACHE_FLASH_ATTR calculateDynamicBitPartLen(dataPointFlags_t *aFlag)
{
    ...
    if(0x01 == aFlag->flagIock)
    {
        bitFieldBitLen += Iock_LEN;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(uint8_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagIock)
    {
        info->event[info->num] = EVENT_Iock;
        info->num++;
        dataPoints->valueIock = gizVarlenDecompressionValue(bitFieldOffset,Iock_LEN,(uint8_t *)&issuedData[DATAPOINT_FLAG_LEN],bitFieldByteLen);
        bitFieldOffset += Iock_LEN;
    }


    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagDiffAge)
    {
        info->event[info->num] = EVENT_DiffAge;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valueDiffAge,(uint8_t *)&issuedData[byteFieldOffset],DiffAge_LEN);
        byteFieldOffset += DiffAge_LEN;
        dataPoints->valueDiffAge = gizX2Y(DiffAge_RATIO,  DiffAge_ADDITION, elongateP0FormTmp.devStatus.valueDiffAge); 
    }

    if(0x01 == elongateP0FormTmp.devDatapointFlag.flagdata)
    {
        info->event[info->num] = EVENT_data;
        info->num++;
        gizMemcpy((uint8_t *)&elongateP0FormTmp.devStatus.valuedata,(uint8_t *)&issuedData[byteFieldOffset],data_LEN);
        byteFieldOffset += data_LEN;
        dataPoints->valuedata = gizX2Y(data_RATIO,  data_ADDITION, exchangeWord(elongateP0FormTmp.devStatus.valuedata));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueIock != cur->valueIock)
    {
        GIZWITS_LOG("valueIock Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagIock = 1;
        ret = 1;
    }
    if(last->valueLatitudinal_direction != cur->valueLatitudinal_direction)
    {
        GIZWITS_LOG("valueLatitudinal_direction Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagLatitudinal_direction = 1;
        ret = 1;
    }
    if(last->valueDirection_of_longitude != cur->valueDirection_of_longitude)
    {
        GIZWITS_LOG("valueDirection_of_longitude Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagDirection_of_longitude = 1;
        ret = 1;
    }
    if(last->valueFS != cur->valueFS)
    {
        GIZWITS_LOG("valueFS Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagFS = 1;
        ret = 1;
    }
    if(last->valueDiffAge != cur->valueDiffAge)
    {
        GIZWITS_LOG("valueDiffAge Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagDiffAge = 1;
        ret = 1;
    }
    if(last->valuedata != cur->valuedata)
    {
        GIZWITS_LOG("valuedata Changed\n");
        gizwitsProtocol.waitReportDatapointFlag.flagdata = 1;
        ret = 1;
    }

    if(last->valuenumSv != cur->valuenumSv)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuenumSv Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagnumSv = 1;
            ret = 1;
        }
    }
    if(last->valueDiffSta_ID != cur->valueDiffSta_ID)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueDiffSta_ID Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagDiffSta_ID = 1;
            ret = 1;
        }
    }
    if(last->valueHDOP != cur->valueHDOP)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueHDOP Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagHDOP = 1;
            ret = 1;
        }
    }
    if(last->valueSep != cur->valueSep)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueSep Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagSep = 1;
            ret = 1;
        }
    }
    if(last->valuetime != cur->valuetime)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuetime Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagtime = 1;
            ret = 1;
        }
    }
    ...
    if(last->valueDirection != cur->valueDirection)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueDirection Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagDirection = 1;
            ret = 1;
        }
    }
    if(last->valueMsl != cur->valueMsl)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMsl Changed\n");
            gizwitsProtocol.waitReportDatapointFlag.flagMsl = 1;
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , uint8_t *outData,uint32_t *outDataLen)
{
    ...
    /*** Fill the bit ***/
    if(gizwitsProtocol.waitReportDatapointFlag.flagIock)
    {
        gizVarlenCompressValue(bitFieldOffset,Iock_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueIock);
        bitFieldOffset += Iock_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagLatitudinal_direction)
    {
        if(dataPoints->valueLatitudinal_direction >= Latitudinal_direction_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valueLatitudinal_direction Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,Latitudinal_direction_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueLatitudinal_direction);
                bitFieldOffset += Latitudinal_direction_LEN;
        }
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagDirection_of_longitude)
    {
        if(dataPoints->valueDirection_of_longitude >= Direction_of_longitude_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valueDirection_of_longitude Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,Direction_of_longitude_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueDirection_of_longitude);
                bitFieldOffset += Direction_of_longitude_LEN;
        }
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagFS)
    {
        if(dataPoints->valueFS >= FS_VALUE_MAX)
        {
            GIZWITS_LOG("[ERROR] valueFS Error , Illegal Overstep\n");
            return -1;
        }
        else
        {
                gizVarlenCompressValue(bitFieldOffset,FS_LEN,(uint8_t *)&allDatapointByteBuf[byteFieldOffset],dataPoints->valueFS);
                bitFieldOffset += FS_LEN;
        }
    }
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagnumSv)
    {
        devStatusTmp.valuenumSv = gizY2X(numSv_RATIO,  numSv_ADDITION, dataPoints->valuenumSv); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuenumSv,numSv_LEN);
        byteFieldOffset += numSv_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagDiffAge)
    {
        devStatusTmp.valueDiffAge = gizY2X(DiffAge_RATIO,  DiffAge_ADDITION, dataPoints->valueDiffAge); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueDiffAge,DiffAge_LEN);
        byteFieldOffset += DiffAge_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagDiffSta_ID)
    {
        devStatusTmp.valueDiffSta_ID = gizY2X(DiffSta_ID_RATIO,  DiffSta_ID_ADDITION, dataPoints->valueDiffSta_ID); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueDiffSta_ID,DiffSta_ID_LEN);
        byteFieldOffset += DiffSta_ID_LEN;
    }

    if(gizwitsProtocol.waitReportDatapointFlag.flagHDOP)
    {
        devStatusTmp.valueHDOP = exchangeBytes(gizY2XFloat(HDOP_RATIO,  HDOP_ADDITION, dataPoints->valueHDOP));     
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueHDOP,HDOP_LEN);
        byteFieldOffset += HDOP_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagSep)
    {
        devStatusTmp.valueSep = exchangeBytes(gizY2X(Sep_RATIO,  Sep_ADDITION, dataPoints->valueSep)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueSep,Sep_LEN);
        byteFieldOffset += Sep_LEN;
    }

    if(gizwitsProtocol.waitReportDatapointFlag.flagtime)
    {
        devStatusTmp.valuetime = exchangeWord(gizY2XFloat(time_RATIO,  time_ADDITION, dataPoints->valuetime)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuetime,time_LEN);
        byteFieldOffset += time_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flaglatitude)
    {
        devStatusTmp.valuelatitude = exchangeWord(gizY2XFloat(latitude_RATIO,  latitude_ADDITION, dataPoints->valuelatitude)); 
    ...
    if(gizwitsProtocol.waitReportDatapointFlag.flagDirection)
    {
        devStatusTmp.valueDirection = exchangeWord(gizY2XFloat(Direction_RATIO,  Direction_ADDITION, dataPoints->valueDirection)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueDirection,Direction_LEN);
        byteFieldOffset += Direction_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagMsl)
    {
        devStatusTmp.valueMsl = exchangeWord(gizY2XFloat(Msl_RATIO,  Msl_ADDITION, dataPoints->valueMsl)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valueMsl,Msl_LEN);
        byteFieldOffset += Msl_LEN;
    }
    if(gizwitsProtocol.waitReportDatapointFlag.flagdata)
    {
        devStatusTmp.valuedata = exchangeWord(gizY2X(data_RATIO,  data_ADDITION, dataPoints->valuedata)); 
        gizMemcpy((uint8_t *)&allDatapointByteBuf[byteFieldOffset],(uint8_t *)&devStatusTmp.valuedata,data_LEN);
        byteFieldOffset += data_LEN;
    }
